...
- 野球: 実装済み
  - 打者9人、投手1人
  - 2025年NPB公式戦成績をもとにした実在チーム・実在選手データ
  - 9イニング制
...
- スポーツ選択
- 野球チームの手動編集
- 年代縛り、所属チーム縛り付きのランダム編成
- seed指定
- 1試合、100試合、1000試合のシミュレーション
- 勝率、平均得点、MVP、イニング別スコア、試合展開、打席ログの表示
- 選手画像、所属履歴、能力詳細、成績ソースリンク、Wikipediaプロフィールリンクの表示

## 選手データと能力値

MVPでは次の4チームを実在チームとして収録しています。
...
能力値は2025年の公式戦成績を0-100評価へ変換した暫定値です。打者は打率、出塁率、長打率、本塁打率、三振率、出場数などから `contact` / `power` / `fielding` を作り、投手は四球率、三振率、被本塁打率、防御率、投球回などから `control` / `stuff` / `stamina` を作っています。

主な参照元:

- [NPB.jp 2025年度 打撃成績](https://npb.jp/bis/2025/stats/)
- [NPB.jp 2025年度 投手成績](https://npb.jp/bis/2025/stats/)

選手画像とプロフィールリンクは日本語版Wikipedia / Wikimedia Commonsのページ画像を使い、画像がない選手だけ生成アバターへフォールバックします。画面上の「成績ソースを見る」から、各選手の所属チーム別成績ページへ移動できます。現時点では守備指標が十分に揃っていないため、守備力は出場数、走力の目安、守備位置の補正から置いたMVP用の暫定値です。
...
- スポーツごとの詳細ルール
- 選手データの外部JSON化
- 打撃、走塁、守備、肩、制球、球威、変化球、スタミナなどの詳細能力化
- 実在シーズン成績をテストデータにした能力係数の本格校正
- Wikipedia / Wikidata など、再利用しやすいデータソースへの差し替え
- リーグ戦、トーナメント
- ユーザー作成チームの保存

## 能力校正の長期方針

現在のMVPは、実在選手の2025年成績から能力を一方向に変換しています。  
次の段階では、同じカードを多数回シミュレーションし、得点、失点、勝率、打率、本塁打、防御率などが実成績に近づくように能力変換式と係数を調整します。

選手詳細には「どの年・どの所属チームの成績を能力の基準にしたか」を表示します。複数チーム・複数年代を扱う場合も、同じ選手IDに複数の所属履歴を持たせ、選択した年代や所属に応じて能力を差し替える方針です。
